...
- The activation acknowledgement test injects a failure after a real PostgreSQL commit. It does not sever the database connection while the server sends the commit response.
- Restart recovery is exercised through repository and runtime boundaries. No maintained test terminates the process at every commit, reconciliation, and load instruction boundary.
