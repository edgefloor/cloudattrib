...
PostgreSQL integration used a fresh pinned PostgreSQL 18 container. It covered migrations, atomic admission, work claims, terminal completion, durable pins, activation crash recovery, and operational metrics.

The dataset repository tests cover malformed and corrupt artifacts, stale sources, single-writer contention, approval-hash-bound activation, rollback, immutable candidates, committed-generation reconciliation, and pruning. Manager, job, runtime, and PostgreSQL tests cover coherent readers during activation, retained last-known-good views, durable activation identity, stale attempt rejection, terminal pin release, capacity reservations, and fail-closed reference checks. Health tests distinguish process liveness, durable-operation storage health, and actual local lookup-index availability.
...
| 4. Activation and pruning | `TestActivateCommittedPublishesOnlyAfterCommit` covers commit-before-publication and idempotent reconciliation. `TestPostgresActivationConvergesAcrossProcessTerminationBoundaries` sends `SIGKILL` before commit, after commit but before filesystem publication, and after reconciliation publishes the pointer but before it appends the audit record. It restarts the helper process and checks convergence by operation ID and generation. `TestPostgresActivationGenerationIsIdempotentAndResolvesLostAcknowledgement` terminates the PostgreSQL backend after commit and covers operation identity. `TestPostgresAdmissionClaimCompletionAndPinLifecycle` covers committed visibility and prune protection. `TestBundleAnalyzerFactoryFailedLoadKeepsLastKnownGood` covers a failed load without replacing the working analyzer. |
...
- The acknowledgement test terminates the backend that executed the activation transaction after PostgreSQL acknowledges the commit. No transport proxy severs the socket while the server's `COMMIT` response is in flight. The exact ambiguous-response interval remains simulated by the post-commit callback.
- The process-termination test covers the protocol boundaries before commit, after commit but before filesystem publication, and after pointer publication during reconciliation. It does not terminate the process at every filesystem instruction or while an analyzer loads.
