...
func (s *server) writeMetrics(writer http.ResponseWriter, request *http.Request) {
	...
	_, _ = fmt.Fprintf(writer, "# TYPE cloudattrib_bundle_resident_generations gauge\ncloudattrib_bundle_resident_generations %d\n", snapshot.ResidentGenerations)
	_, _ = fmt.Fprintf(writer, "# TYPE cloudattrib_bundle_estimated_retained_bytes gauge\ncloudattrib_bundle_estimated_retained_bytes %d\n", snapshot.EstimatedRetainedBytes)
