...
func TestSPFIncludeRequiresReachablePositiveMechanism(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "valid domain spec and CIDR mechanisms", value: "v=spf1 a:mail.example.com/24/64 mx:mail.example.com/24/64 ptr:mail.example.com exists:%{l}.example.com include:_spf.google.com -all", match: true},
